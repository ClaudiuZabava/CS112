...
if afn==1: #Daca inputul a fost valid, AFN-ul poate fi convertit la un AFD.
    ...
    '''     Algoritmul de mai jos implica si testarea unui cuvant pe noul AFD pentru a fi siguri ca functioneaza : '''


    c=sys.argv[2] if len(sys.argv)>2 else input() # atunci cand se doreste rularea din command line, c retine argumentul din CMD

    s_init = s_start  # initializam starea initiala cu starea de inceput
...
# Acest cod a fost realizat de: Zabava Claudiu
